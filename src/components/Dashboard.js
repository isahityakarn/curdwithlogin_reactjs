...
import generateNocPdf from './generateNocPdf';

const Dashboard = () => {

  const { user } = useAuth();
  const { nocData, loading: nocLoading, error: nocError } = useNocData(3);
  ...
  if (nocLoading) return <div>Loading NOC data...</div>;
  if (nocError) return <div>Error loading NOC data.</div>;
  return (
